...
from pace.dsl.typing import FloatField, BoolFieldIJ, FloatFieldIJ
...
def stability(z1, snwdph, thv1, wind, z0max, ztmax, tvs,
    ...
    if dtv >= 0.0:
        ...
        dtv = -abs(adtv)
    rb = max(-5000.0, (constants.GRAV+constants.GRAV) * dtv * z1 / ((thv1 + tvs) * wind * wind))
    tem1 = 1.0 / z0max
    tem2 = 1.0 / ztmax
    ...
    with computation(FORWARD), interval(0,1):
        ...
        if flag_iter[0,0]:
            ...
            if dry[0,0]:
                ...
                tem1 = 1.0 - sigmaf[0,0]
                ztmax = z0max*exp(-tem1*tem1 * czilc*constants.CA*sqrt(ustar[0,0,0] * (0.01/1.5e-5)))
                ...
                cm[0,0,0], ch[0,0,0], stress[0,0,0], ustar[0,0,0] = \
                    stability(z1[0,0], snwdph[0,0,0], thv1, wind[0,0], z0max, ztmax, tvs,
                            rb[0,0,0], fm[0,0,0], fh[0,0,0], fm10[0,0,0], fh2[0,0,0],
                            cm[0,0,0], ch[0,0,0], stress[0,0,0], ustar[0,0,0])
    ...
    with computation(FORWARD), interval(2,3):
        if flag_iter[0,0]:
            ...
            if wet[0,0]:
                ...
                if sfc_z0_type == 0:
                    z0 = (constants.CHARNOCK / constants.GRAV) * ustar[0,0,0] * ustar[0,0,0]
