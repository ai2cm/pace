...
class DycoreState:
    ...
    diss_estd: FloatField = field(metadata={"name": "dissipation_estimate_from_heat_source", "dims": [fv3util.X_DIM, fv3util.Y_DIM, fv3util.Z_DIM], "units": "unknown", "n_halo":(3, 3), "intent":"inout"})
    ...
    def init_from_quantities(cls, dict_of_quantities):
        ...
        for field in fields(cls):
            for varcheck in ["units", "dims"]:
                if field.metadata[varcheck] != dict_of_quantities[field.name][varcheck]:
                    raise TypeError(field_name + ' has metadata ' + varcheck + ' of ' + dict_of_quantities[field.name][varcheck] + ' that does not match the requirement' +  field.metadata[varcheck])
