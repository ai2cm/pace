...
from pace.util.grid.gnomonic import get_lonlat_vect, get_unit_vector_direction, lon_lat_midpoint
...
def init_tc_state(metric_terms: MetricTerms,
    ...
    ud, vd = _initialize_wind_dgrid(metric_terms, tc_properties, calc, pe_u, pe_v, ps_u, ps_v, shape)
    ua, va = _interpolate_winds_dgrid_agrid(metric_terms, ud, vd, tc_properties, shape)

    qvapor, pt = _initialize_qvapor_temperature(metric_terms, pe, ps, tc_properties, calc, shape)
...
def _calculate_distance_from_tc_center(pe_v, ps_v, muv, calc, tc_properties):
    ...
    d1 = np.sin(calc["p0"][1]) * np.cos(muv["midpoint"][:, :, 1]) - np.cos(calc["p0"][1]) * np.sin(muv["midpoint"][:, :, 1]) * np.cos(muv["midpoint"][:, :, 0]) - calc["p0"][0]
    d2 = np.cos(calc["p0"][1]) * np.sin(muv["midpoint"][:, :, 0]) - calc["p0"][0]
    d = np.sqrt(d1**2 + d2**2)
    d[d < 1e-15] = 1e-15

    r = great_circle_distance_lon_lat(calc["p0"][0], muv["midpoint"][:, :, 0], calc["p0"][1], muv["midpoint"][:, :, 1], constants.RADIUS, np)
    ptmp = 0.5 * (pe_v[:, :, :-1] + pe_v[:, :, 1:])
    height = (calc["t00"] / tc_properties["gamma"]) * (1.0 - (ptmp/ps_v[:, :, None])**calc["exponent"])
...
def _calculate_pt_height(height, qvapor, r, tc_properties, calc):
    ...
    cc = np.exp((height / tc_properties["zp"])**tc_properties["exppz"]) * np.exp((r[:, :, None] / tc_properties["rp"])**tc_properties["exppr"])
...
def _calculate_utmp(height, r, d, calc, tc_properties):
    ...
    bb = (1.0 - tc_properties["p_ref"] / tc_properties["dp"] * np.exp(r_ratio)[:, :, None] * aa)
    cc = (tc_properties["exppz"] * height * constants.RDGAS * (calc["t00"] - tc_properties["gamma"] * height) / (constants.GRAV * tc_properties["zp"]**tc_properties["exppz"]) + bb)
    dd = np.sqrt((calc["cor"] * r[:, :, None] / 2.0)**(2.0) - tc_properties["exppr"] * r_ratio[:, :, None] * constants.RDGAS * (calc["t00"] - tc_properties["gamma"] * height) / cc)

    utmp = 1. / d[:, :, None] * (-calc["cor"] * r[:, :, None] / 2.0 + dd)
...
def _calculate_vortex_surface_pressure_with_radius(p0, p_grid, tc_properties):
    ...
    r = great_circle_distance_lon_lat(p0[0], p_grid[:, :, 0], p0[1], p_grid[:, :, 1], constants.RADIUS, np)
    ps = tc_properties["p_ref"] - tc_properties["dp"] * np.exp(-(r/tc_properties["rp"])**1.5) 
...
def _find_midpoint_unit_vectors(p1, p2):
    ...
    midpoint = np.array(lon_lat_midpoint(p1[:, :, 0], p2[:, :, 0], p1[:, :, 1], p2[:, :, 1], np)).transpose([1, 2, 0])
...
def _initialize_delz_w(pe, pt, qvapor, tc_properties, shape):
    ...
    if tc_properties["hydrostatic"] is False:
        delz[:, :, :-1] = constants.RDGAS * pt[:, :, :-1] * (1 + constants.ZVIR * qvapor[:, :, :-1]) / constants.GRAV * np.log(pe[:, :, :-1]/pe[:, :, 1:])
        w[:] = 0.0
...
def _initialize_qvapor_temperature(metric_terms, pe, ps, tc_properties, calc, shape):
    ...
    ptmp = 0.5 * (pe[:, :, :-1] + pe[:, :, 1:])
    height = (calc["t00"] / tc_properties["gamma"]) * (1. - (ptmp / ps[:, :, None])**calc["exponent"])

    qvapor[:, :, :-1] = tc_properties["q00"] * np.exp(-height / tc_properties["zq1"]) * np.exp(- (height / tc_properties["zq2"])**tc_properties["exppz"])

    p2 = metric_terms.agrid.data
    r = great_circle_distance_lon_lat(calc["p0"][0], p2[:, :, 0], calc["p0"][1], p2[:, :, 1], constants.RADIUS, np)
...
def _initialize_vortex_ps_phis(metric_terms, shape, tc_properties, calc):
    ...
    ps = np.zeros(shape[:2])
    ps = _calculate_vortex_surface_pressure_with_radius(calc["p0"], metric_terms.agrid.data, tc_properties)

    ps_vc = np.zeros(shape[:2])
    p_grid = 0.5 * (metric_terms.grid.data[:, :-1, :] + metric_terms.grid.data[:, 1:, :])
    ps_vc[:, :-1] = _calculate_vortex_surface_pressure_with_radius(p0, p_grid, tc_properties)

    ps_uc = np.zeros(shape[:2])
    p_grid = 0.5 * (metric_terms.grid.data[:-1, :, :] + metric_terms.grid.data[1:, :, :])
    ps_uc[:-1, :] = _calculate_vortex_surface_pressure_with_radius(p0, p_grid, tc_properties)
...
def _initialize_wind_dgrid(metric_terms, tc_properties, calc, pe_u, pe_v, ps_u, ps_v, shape):
    # u-wind
    ud = np.zeros(shape)
    ...
    utmp = _calculate_utmp(dist["height"][:-1, :, :], dist["r"], dist["d"], calc, tc_properties)
    vtmp = utmp * dist["d2"][:, :, None]
    ud[:-1, :, :-1] = utmp * np.sum(muv["unit_dir"] * muv["exv"], 2)[:, :, None] + vtmp * np.sum(muv["unit_dir"] * muv["eyv"], 2)[:, :, None]
    ...
    vd[:, :-1, :-1] = utmp * np.sum(muv["unit_dir"] * muv["exv"], 2)[:, :, None] + vtmp * np.sum(muv["unit_dir"] * muv["eyv"], 2)[:, :, None]
...
def _interpolate_winds_dgrid_agrid(metric_terms, ud, vd, tc_properties, shape):
    ...
    if tc_properties["vort"] is True:
        ua[:, :-1, :] = 0.5 * (ud[:, :-1, :] * metric_terms.dx.data[:, :-1, None] + ud[:, 1:, :] * metric_terms.dx.data[:, 1:, None]) / metric_terms.dxa.data[:, :-1, None]
        va[:-1, :, :] = 0.5 * (vd[:-1, :, :] * metric_terms.dy.data[:-1, :, None] + vd[1:, :, :] * metric_terms.dy.data[1:, :, None]) / metric_terms.dya.data[:-1, :, None]
    else:
        pass
...
def _some_inital_calculations(tc_properties):
    t00 = tc_properties["Ts0"] * (1. + constants.ZVIR * tc_properties["q00"]) # num
    p0 = [np.deg2rad(tc_properties["lon_tc"]), np.deg2rad(tc_properties["lat_tc"])]
    exponent = constants.RDGAS * tc_properties["gamma"] / constants.GRAV # num
    cor = 2. * constants.OMEGA * np.sin(np.deg2rad(tc_properties["lat_tc"])) # num
