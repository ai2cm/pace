...
import os
import sys
sys.path.append('/serialbox2/install/python')
...
def get_test_class_instance(test_name, split_name, grid):
    ...
    test_class_name = 'fv3.translate.translate_' + package_name.lower()  + '.Translate' + test_name.replace("-", "_")
...
def collect_input_data(testobj, serializer, sp, sp_grid):
    input_data = {}
    for k in testobj.serialnames(testobj.in_vars['data_vars']) + testobj.in_vars['parameters']:
    ...
    if print_failures:
        ...
        for b in range(bad_indices_count):
            if bad_count%failure_stride == 0:
                ...
                print(test_str, 'index:',full_index,computed_data[found_indices][b], ref_data[found_indices][b], abs(computed_data[found_indices][b]- ref_data[found_indices][b]))
            bad_count += 1
    sample = [f[0] for f in found_indices]
    fullcount = len(ref_data.flatten())
    return "Failed count: " + str(bad_indices_count) + "/" +str(fullcount) +  "(" + str(round(100.0*(bad_indices_count / fullcount), 2))+ "%), First failed index" + str(sample) + ", Computed value: " + str(computed_data[found_indices][0]) +\
        ', Reference serialized value: ' + str(ref_data[found_indices][0]) + \
        ', Diff:' + str(abs(computed_data[found_indices][0] - ref_data[found_indices][0]))
...
def data_path(pytestconfig):
    os.environ['NAMELIST_FILENAME'] = os.path.join(pytestconfig.getoption("data_path"), 'input.nml')
...
def interpret_success(test_name, var, ref_data, testobj, test_str, args):
    if success(args['output_data'][test_name][var], ref_data, testobj.max_error):
        ...
        if success(args['output_data'][test_name][var] * 1.0001,  ref_data, testobj.max_error):
...
def process_output_savepoint(serializer, sp, testobj, test_name, args):
    ...
    for var in testobj.serialnames(testobj.out_vars):
        test_str = "rank: " + str(args['rank']) + ", savepoint: " + sp.name + ", ID: " + str(sp.metainfo['ID']) + ", variable: " + var
...
def process_test_savepoint(serializer, sp, split_name, args):
    ...
    if (args['which_modules'] != ['all'] and test_name not in args['which_modules']) or test_name in args['skip_modules']:
        return 0
...
def test_serialized_savepoints(which_modules, skip_modules, print_failures, failure_stride,  data_path, exec_backend, data_backend, backend):
    ...
    for rank in range(6):
        ...
        args.update({'failure_stride': failure_stride, 'isready': [],
                     'unimplemented': [], 'output_data': {}, 'rank': rank})
