...
class TranslateFortranData2Py:
    ...
    def make_storage_data_input_vars(self, inputs, storage_vars=None):
        ...
        for d, info in storage_vars.items():
            ...
            inputs[d] = self.make_storage_data(np.squeeze(inputs[serialname]), istart=istart, jstart=jstart, kstart=kstart)
            if d != serialname:
    ...
    def serialnames(self, dict):
        return [info['serialname'] if 'serialname' in info else d for d, info in dict.items()]
...
class TranslateGrid:
    ...
    def make_grid_storage(self, pygrid):
        ...
        for k, axis in TranslateGrid.edge_var_axis.items():
            if k in self.data:
                self.data[k] = utils.make_storage_data_from_1d(self.data[k], shape, kstart=pygrid.halo, axis=axis)
