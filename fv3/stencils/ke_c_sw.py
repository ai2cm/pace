...
def update_vorticity_edge_values( vort: sd, va: sd, u: sd, sin: sd, cos: sd ): 
    with computation(PARALLEL), interval(...):
        vort = vort*sin + u*cos if va > 0.0 else vort

@gtscript.stencil(backend=utils.exec_backend, rebuild=True)
def update_vorticity_outer_edge_values( vort: sd, va: sd, u: sd, sin: sd, cos: sd ): 
    with computation(PARALLEL), interval(...):
        vort = vort*sin + u[0,1,0]*cos if va <= 0.0 else vort
...
def update_ke_edge_values( ke: sd, ua: sd, v: sd, sin: sd, cos: sd ):
    with computation(PARALLEL), interval(...):
        ke = ke*sin + v*cos if ua > 0.0 else ke 

@gtscript.stencil(backend=utils.exec_backend, rebuild=True)
def update_ke_outer_edge_values( ke: sd, ua: sd, v: sd, sin: sd, cos: sd ):
    with computation(PARALLEL), interval(...):
        ke = ke*sin + v[1,0,0]*cos if ua <= 0.0 else ke
...
def compute(uc, vc, u, v, ua, va, dt2):
    ...
    if spec.namelist['grid_type'] < 3 and not grid.nested:
       ...
       if grid.south_edge:
          update_vorticity_outer_edge_values( vort_c, va, u, grid.sin_sg4, grid.cos_sg4, origin=origin, domain=vort_domain )
          update_vorticity_edge_values( vort_c, va, u, grid.sin_sg2, grid.cos_sg2, origin=(grid.is_-1,grid.js,0), domain=vort_domain )

       if grid.north_edge:
          update_vorticity_outer_edge_values( vort_c, va, u, grid.sin_sg4, grid.cos_sg4, origin=(grid.is_-1,grid.npy+1,0), domain=vort_domain )
          update_vorticity_edge_values( vort_c, va, u, grid.sin_sg2, grid.cos_sg2, origin=(grid.is_-1,grid.npy+2,0), domain=vort_domain )

       ke_domain = (1,grid.npy+3,grid.npz)
       if grid.east_edge:
          update_ke_outer_edge_values( ke_c, ua, v, grid.sin_sg3, grid.cos_sg3, origin=(grid.npx+1,grid.js-1,0), domain=ke_domain )
          update_ke_edge_values( ke_c, ua, v, grid.sin_sg1, grid.cos_sg1, origin=(grid.npx+2,grid.js-1,0), domain=ke_domain )

       if grid.west_edge:
          update_ke_outer_edge_values( ke_c, ua, v, grid.sin_sg3, grid.cos_sg3, origin=(grid.is_-1,grid.js-1,0), domain=ke_domain )
          update_ke_edge_values( ke_c, ua, v, grid.sin_sg1, grid.cos_sg1, origin=(grid.is_,grid.js-1,0), domain=ke_domain )

    # Update kinetic energy field using computed vorticity
    update_kinetic_energy( ke_c, vort_c, ua, va, dt2, origin=origin, domain=(grid.nic+2,grid.njc+2,grid.npz) )
    return ke_c, vort_c
