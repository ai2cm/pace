...
def al_x_edge(q: sd, dya: sd, al: sd):
    with computation(PARALLEL), interval(0, None):
        ...
        al[0, 1, 0] = 0.5 * (((2.0 * dya + dya[0, -1, 0]) * q
                              - dya * q[0, -1, 0]) / (dya[0, -1, 0] + dya)
                             + ((2.0 * dya[0, 1, 0] + dya[0, 2, 0]) * q[0, 1, 0]
                                - dya[0, 1, 0] * q[0, 2, 0]) / (dya[0, 1, 0] + dya[0, 2, 0]))
        al[0, 2, 0] = c3 * q[0, 1, 0] + c2 * q[0, 2, 0] + c1 * q[0, 3, 0]
...
def get_flux(q, c, al, mord):
    ...
    flux = q[0, -1, 0] + fx1 * tmp if c > 0.0 else q + fx1 * tmp #final_flux(c, q, fx1, tmp)
    return flux
