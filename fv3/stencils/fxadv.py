...
def main_ut(uc: sd, vc: sd, cosa_u: sd, rsin_u: sd, ut: sd):
    with computation(PARALLEL), interval(0, None):
        ut = (uc - 0.25 * cosa_u * (vc[-1, 0, 0] + vc + vc[-1, 1, 0] + vc[0, 1, 0])) * rsin_u
...
def main_vt(uc: sd, vc: sd, cosa_v: sd, rsin_v: sd, vt: sd):
    with computation(PARALLEL), interval(0, None):
        vt = (vc - 0.25 * cosa_v * (uc[0, -1, 0] + uc[1, -1, 0] + uc + uc[1, 0, 0])) * rsin_v
...
@gtscript.stencil(backend=utils.exec_backend, rebuild=True)
def xfx_adv_stencil(ut: sd, rdxa: sd, area: sd, dy: sd, sin_sg1: sd, sin_sg3: sd,  crx_adv: sd, xfx_adv: sd, ra_x: sd, dt: float):
    with computation(PARALLEL), interval(...):
        xfx_adv = dt * ut
        crx_adv = xfx_adv * rdxa[-1, 0, 0] if xfx_adv > 0 else xfx_adv * rdxa
        xfx_adv = dy * xfx_adv * sin_sg3[-1, 0, 0] if xfx_adv > 0 else dy * xfx_adv * sin_sg1
        ra_x = area + xfx_adv - xfx_adv[1, 0, 0]

@gtscript.stencil(backend=utils.exec_backend, rebuild=True)
def yfx_adv_stencil(vt: sd, rdya: sd, area:sd, dx: sd, sin_sg2: sd, sin_sg4: sd,  cry_adv: sd, yfx_adv: sd, ra_y: sd,  dt: float):
    with computation(PARALLEL), interval(...):
        yfx_adv = dt * vt
        cry_adv = yfx_adv * rdya[0, -1, 0] if yfx_adv > 0 else yfx_adv * rdya
        yfx_adv = dx * yfx_adv * sin_sg4[0, -1, 0] if yfx_adv > 0 else dx * yfx_adv * sin_sg2
...
def compute(uc_in, vc_in, ut_in, vt_in, xfx_adv, yfx_adv, crx_adv, cry_adv, dt):
    ...
    vt = compute_vt(uc_in, vc_in, grid.cosa_v, grid.rsin_v, grid.sin_sg2, grid.sin_sg4, vt_in)
    ...
    xorigin = (grid.is_, grid.jsd, 0)
    #xfx_adv = utils.make_storage_from_shape(uc_in.shape, xorigin)
    #crx_adv = utils.make_storage_from_shape(uc_in.shape, xorigin)
    ra_x = utils.make_storage_from_shape(uc_in.shape, xorigin)
    xfx_adv_stencil(ut, grid.rdxa, grid.area, grid.dy, grid.sin_sg1, grid.sin_sg3,  crx_adv, xfx_adv, ra_x, dt,
                    origin=xorigin, domain=(grid.nic + 1, grid.njd, grid.npz))
    yorigin = (grid.isd, grid.js, 0)
    #yfx_adv = utils.make_storage_from_shape(vc_in.shape, yorigin)
    #cry_adv = utils.make_storage_from_shape(vc_in.shape, yorigin)
    ra_y = utils.make_storage_from_shape(vc_in.shape, yorigin)
    yfx_adv_stencil(vt, grid.rdya, grid.area, grid.dx, grid.sin_sg2, grid.sin_sg4,  cry_adv, yfx_adv, ra_y, dt,
                    origin=yorigin, domain=(grid.nid, grid.njc+1, grid.npz))
...
def compute_ut(uc_in, vc_in, cosa_u, rsin_u, ut_in):
    ...
    main_ut(uc_in, vc_in, cosa_u, rsin_u, ut, origin=ut_origin, domain=(grid.nic + 3, grid.njd, grid.npz))
...
def compute_vt(uc_in, vc_in, cosa_v, rsin_v, sin_sg2, sin_sg4, vt_in):
    ...
    main_j_size = grid.jed - grid.jsd - 2
    main_vt(uc_in, vc_in, cosa_v, rsin_v, vt, origin=vt_origin, domain=(grid.nid, grid.njc + 3, grid.npz))#, origin=(0, 2, 0), domain=(vt.shape[0]-1, main_j_size, vt.shape[2]))
...
def corner_ut_stencil(uc: sd, vc: sd, ut: sd, vt: sd,  cosa_u: sd, cosa_v: sd):
    ...
    with computation(PARALLEL), interval(...):
        ut[0, 0, 0] = (uc[0, 0, 0] - 0.25 * cosa_u[0, 0, 0] *
                       (vt[vi, vy, 0] +
                        vt[vx, vy, 0] +
                        vt[vx, vj, 0] +
                        vc[vi, vj, 0] -
                        0.25 * cosa_v[vi, vj, 0] * (ut[ux, 0, 0] +
                                                    ut[ux, uy, 0] +
                                                    ut[0, uy, 0]))) * \
                                                    1.0 / (1.0 - 0.0625 * cosa_u[0, 0, 0] * cosa_v[vi, vj, 0])
...
def corner_ut(uc, vc, ut, vt,  cosa_u, cosa_v,  ui, uj, vi, vj, west, lower, south=True, vswitch=False):
    if vswitch:
    ...
    if stencil_corner:
        decorator = gtscript.stencil(
            utils.exec_backend, externals={"vi": vi-ui, "vj": vj-uj,
                                'ux': ux-ui, 'uy': uy-uj, 'vx': vx-ui, 'vy': vy-uj}, rebuild=True
                   )
        corner_stencil = decorator(corner_ut_stencil)
        corner_stencil(uc, vc, ut, vt,  cosa_u, cosa_v, origin=(ui, uj, 0), domain=(1, 1, grid.npz))
    else:
        damp = get_damp(cosa_u, cosa_v, ui, uj, vi, vj)
        ut[ui, uj, :] = (uc[ui, uj, :] - 0.25 * cosa_u[ui, uj, :] *
                         (vt[vi, vy, :] +
                          vt[vx, vy, :] +
                          vt[vx, vj, :] +
                          vc[vi, vj, :] -
                          0.25 * cosa_v[vi, vj, :] * (ut[ux, uj, :] +
                                                      ut[ux, uy, :] +
                                                      ut[ui, uy, :]))) * damp
...
def sw_corner(uc, vc, ut, vt, cosa_u, cosa_v, corner_shape):
    ...
    corner_ut(vc, uc, vt, ut, cosa_v, cosa_u, z, t, z, n, west=True, lower=True, vswitch=True)
    corner_ut(uc, vc, ut, vt, cosa_u, cosa_v, t, n, n, t, west=True, lower=False)
    corner_ut(vc, uc, vt, ut, cosa_v, cosa_u, n, t, t, n, west=True, lower=False, vswitch=True)
...
def se_corner(uc, vc, ut, vt, cosa_u, cosa_v, corner_shape):
    ...
    z = grid.js - 1
    m = grid.js - 2
    corner_ut(uc, vc, ut, vt,  cosa_u, cosa_v, grid.ie, z, grid.ie, z, west=False, lower=True)
    corner_ut(vc, uc, vt, ut,  cosa_v, cosa_u,  grid.ie+1, t, grid.ie+2, n, west=False, lower=True, vswitch=True)
    corner_ut(uc, vc, ut, vt,  cosa_u, cosa_v, grid.ie, n, grid.ie, t, west=False, lower=False)
    corner_ut(vc, uc, vt, ut,  cosa_v, cosa_u,  grid.ie, t, grid.ie, n, west=False, lower=False, vswitch=True)


def ne_corner(uc, vc, ut, vt, cosa_u, cosa_v, corner_shape):
    corner_ut(uc, vc, ut, vt,  cosa_u, cosa_v, grid.ie, grid.je+1, grid.ie, grid.je+2, west=False, lower=False)
    corner_ut(vc, uc, vt, ut,  cosa_v, cosa_u,  grid.ie+1, grid.je, grid.ie+2, grid.je, west=False, lower=False,
              south=False, vswitch=True)
    corner_ut(uc, vc, ut, vt,  cosa_u, cosa_v, grid.ie, grid.je, grid.ie, grid.je, west=False, lower=True)
    corner_ut(vc, uc, vt, ut,  cosa_v, cosa_u, grid.ie, grid.je, grid.ie, grid.je, west=False, lower=True,
              south=False, vswitch=True)
...
def nw_corner(uc, vc, ut, vt, cosa_u, cosa_v, corner_shape):
    ...
    z = grid.js - 1
    m = grid.js - 2
    corner_ut(uc, vc, ut, vt,  cosa_u, cosa_v, t, grid.je+1, n, grid.je+2, west=True, lower=False)
    corner_ut(vc, uc, vt, ut,  cosa_v, cosa_u, z, grid.je, z, grid.je, west=True, lower=False, south=False,
              vswitch=True)
    corner_ut(uc, vc, ut, vt,  cosa_u, cosa_v, t, grid.je, n, grid.je, west=True, lower=True)
    corner_ut(vc, uc, vt, ut, cosa_v, cosa_u, n, grid.je, t, grid.je, west=True, lower=True, south=False,
              vswitch=True)
