...
@gtscript.stencil(backend=utils.exec_backend, rebuild=True)
def compute_uf( uf: sd, u: sd, va: sd, cos_sg4: sd, cos_sg2: sd, dyc: sd, sin_sg4: sd, sin_sg2: sd ):
    with computation(PARALLEL), interval(...):
         uf = (u - 0.25*(va[0,-1,0] + va)*(cos_sg4[0,-1,0] + cos_sg2)) * dyc*0.5*(sin_sg4[0,-1,0] + sin_sg2)

@gtscript.stencil(backend=utils.exec_backend, rebuild=True)
def compute_vf( vf: sd, v: sd, ua: sd, cos_sg3: sd, cos_sg1: sd, dxc: sd, sin_sg3: sd, sin_sg1: sd ):
    with computation(PARALLEL), interval(...):
         vf = (v - 0.25*(ua[-1,0,0] + ua)*(cos_sg3[-1,0,0] + cos_sg1))*dxc*0.5*(sin_sg3[-1,0,0] + sin_sg1)
