...
def storage_dict_from_var_list(var_list, serializer, savepoint, max_shape, start_indices, axes={}):
    d = {}
    for var in var_list:
...
@gtscript.stencil(backend=BACKEND)
def copy3_stencil(in_field1: FIELD_FLT, in_field2: FIELD_FLT, in_field3: FIELD_FLT, out_field1: FIELD_FLT, out_field2: FIELD_FLT, out_field3: FIELD_FLT):
    with computation(PARALLEL), interval(...):
...
for tile in range(6):
    ...
    max_shape = (index_data["ied"] - index_data["isd"] + 2, index_data["jed"] - index_data["jsd"] + 2,index_data["npz"]+1)
    start_indices = {"vlon": (index_data["isd"] + 1, index_data["jsd"] + 1), "vlat": (index_data["isd"]+1, index_data["jsd"]+1)}
    axes = {"edge_vect_s": 0, "edge_vect_n": 0, "edge_vect_w": 1, "edge_vect_e": 1}
    # read serialized input data
    in_data = storage_dict_from_var_list(IN_VARS, serializer, in_savepoint, max_shape, start_indices, axes)
    ...
    ref_data = storage_dict_from_var_list(OUT_VARS, serializer, out_savepoint, max_shape, start_indices, axes)
