...
def scattered_quantities(tile_quantity, layout, n_rank_halo, numpy):
    ...
    for rank in range(total_ranks):
        ...
        subtile_slice = partitioner.subtile_slice(
            rank, tile_quantity.metadata, overlap=True
        )
        subtile_view = tile_quantity.view[subtile_slice]
        subtile_quantity = get_quantity(
            tile_quantity.dims, tile_quantity.units, subtile_view.shape, n_rank_halo, numpy
        )
...
def test_tile_scatter_no_recv_quantity(tile_quantity, scattered_quantities, communicator_list):
    result_list = []
...
def test_tile_scatter_with_recv_quantity(tile_quantity, scattered_quantities, communicator_list):
    ...
    for recv, communicator in zip(recv_quantities, communicator_list):
        if communicator.rank == 0:
            result = communicator.scatter(send_quantity=tile_quantity, recv_quantity=recv)
        else:
            result = communicator.scatter(recv_quantity=recv)
        assert result is recv
    for rank, (result, scattered) in enumerate(zip(recv_quantities, scattered_quantities)):
        assert result.dims == scattered.dims
        assert result.units == scattered.units
        assert result.extent == scattered.extent
        scattered.np.testing.assert_array_equal(result.view[:], scattered.view[:])
...
def test_tile_scatter_state_with_recv_state(tile_quantity, scattered_quantities, communicator_list, time):
    tile_state = {
        'time': time,
        'air_temperature': tile_quantity
    }
