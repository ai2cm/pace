...
class Driver:
    def __init__(self, namelist, comm, backend, dycore_init_mode='serialized_data', rebuild=False, validate_args=True, data_dir=None):
        self._data_dir = data_dir
        self.namelist = namelist
        self.backend = backend
        partitioner = pace.util.CubedSpherePartitioner(pace.util.TilePartitioner(self.namelist.layout))
        self.comm = pace.util.CubedSphereCommunicator(comm, partitioner)
        self.quantity_factory, self.stencil_factory = self.setup_factories(rebuild, validate_args)
        ...
        self.dycore = fv3core.DynamicalCore(
            ...
            damping_coefficients=DampingCoefficients.new_from_metric_terms(self.metric_terms),
            config=self.namelist.dynamical_core,
            phis=self.dycore_state.phis, 
        )
        if not self.namelist.dycore_only:
            self.physics = Physics(stencil_factory=self.stencil_factory,   grid_data=self.grid_data, namelist=self.namelist)
            self.physics_state = self.init_physics_from_dycore()
            self.state_updater =  UpdateAtmosphereState(
                stencil_factory=self.stencil_factory, grid_data=self.grid_data, namelist=self.namelist, comm=self.comm, grid_info=DriverGridData.new_from_metric_terms(self.metric_terms), quantity_factory=self.quantity_factory
            )
    ...
    def compute_grid(self):
        metric_terms = MetricTerms(quantity_factory=self.quantity_factory, communicator=self.comm)
    ...
    def initialize_dycore_state(self, dycore_init_mode):
        if dycore_init_mode == 'serialized_data':
            ...
            serializer = serialbox.Serializer(
                serialbox.OpenModeKind.Read, self._data_dir, "Generator_rank" + str(self.comm.rank),
            )
            dycore_state = init_dycore_state_from_serialized_data(serializer=serializer, rank=self.comm.rank, backend=self.backend, namelist=self.namelist.dynamical_core, quantity_factory=self.quantity_factory, stencil_factory=self.stencil_factory)
            ...
            raise ValueError("dycore_init_mode is " + dycore_init_mode + ", which is not one of the supported options")
        return dycore_state
    ...
    def init_physics_from_dycore(self):
        ...
        for field in fields(PhysicsState):
            if field.metadata["full_model_var"]:
                ...
                initial_storages[field.name] = self.quantity_factory.zeros([pace.util.X_DIM, pace.util.Y_DIM, pace.util.Z_DIM],  field.metadata["units"], dtype=float)
