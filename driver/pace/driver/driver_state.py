...
class DriverState:
    ...
    def init_empty(cls, grid, quantity_factory, namelist, comm, grid_info):
        ...
        return cls(
            dycore_state, physics_state, quantity_factory, namelist, comm, grid_info
        )

    # intended for the case of not copying from physics to dycore, reusing
    # the same variables in the dycore and physics
    @classmethod
    def init_physics_from_dycore(
        cls, quantity_factory: fv3util.QuantityFactory, dycore_state: DycoreState
    ):
        initial_storages = {}
    ...
    def init_from_quantities(
        ...
        return cls.init_from_dycore_state(
            dycore_state, quantity_factory, namelist, comm, grid_info
        )

    def update_physics_inputs_state(self):
        self.state_updater.copy_from_dycore_to_physics(
            self.dycore_state, self.physics_state
        )
