...
for t in range(2):
    ...
    h = pcolormesh_cube(
        lat,
        lon,
        ds[args.variable][:, :, 0:192, 0:192, :].isel(time=t, z=args.zlevel).values,
        cmap=plt.cm.viridis,
        ax=ax,
    )
    fig.colorbar(h, ax=ax, location="bottom", label=f"{args.variable}")
    title = args.experiment.replace("_", " ")
    fig.suptitle(f"{title}: {args.variable}, z={args.zlevel}, t={str(t)}")
    ax.annotate(
    ...
    plt.tight_layout()
    plt.savefig(
        f"/work/{args.experiment}_baroclinic_initialization_{args.variable}_time_{str(t)}.png",
        dpi=150,
    )
    plt.close()
